...
- Crudos: `panel/crudos.js` + `functions/api/crudos/[[ruta]].js` suben a R2 (binding `CRUDOS`) en partes de 25 MB, sin tope. Al terminar crean un pedido "Editar crudo" con el curl para bajarlo (`/api/crudos/bajar?key=`, con `x-pin`). Sin R2 activado responde 503 `sin_r2`.
